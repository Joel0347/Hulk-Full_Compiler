...
State epsilon_clousure(NFA* nfa, int* states, int count) {
    ...
    set->count = count_set;
    set->set = (int*)malloc(sizeof(int) * count_set);
...
void copy_state_set(State* dest, State* src) {
    ...
    for (int i = 0; i < src->matches; i++) {
        dest->tokens[i] = strdup(src->tokens[i]);
    }
}
...
DFA* nfa_to_dfa(NFA* nfa) {
    ...
    while (index>0)
    {
        ...
        for (int s = 0; s < symbols_count; s++) {
            ...
            int states_count = 0;
            for (int q = 0; q < Q.count; q++) { 
                for (int t = 0; t < nfa->transitions_count; t++) { 
    ...
    for (int i = 0; i < nfa->finals_count; i++) { 
        for (int j = 0; j < dfa->states_count; j++) { 
    ...
    //     DFA_Transition t = dfa->transitions[i];
        ...
        //     printf("%d, ", t.to.set[j]);
        // }
        // if (t.to.matches == 0) printf("tokens: %d\n", t.to.matches);
